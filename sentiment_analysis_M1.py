...
def analyze_sentiment_batch(model, data_loader, device):
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(data_loader, desc="Processing batches")):
            ...
            # label = ['negative' if (x == 0 and conf >= threshold) else 'positive' for x, conf in zip(label_id.cpu().numpy(), confidence)]
...
def load_model(model_name, local_dir, num_labels=2):
...
def main():
    # Load the CSV file
    file_path = '/Users/nimaazar/Desktop/sample.csv'  # Update with the correct path to your file
    ...
    local_dir = "/Users/nimaazar/Desktop/local_model_cache"
    ...
    labels, confidences = analyze_sentiment_batch(model, data_loader, device)  # Adjust threshold if needed
    df['label'] = labels
    df['confidence'] = confidences

    # Save the updated DataFrame
    output_path = '/Users/nimaazar/Desktop/final.csv'  # Update with the desired output path
